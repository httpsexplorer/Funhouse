...
local function safeCall(name, fn)
	print("[Pink's] safeCall START:", tostring(name))
	local ok, err = pcall(fn)
	if ok then
		print("[Pink's] safeCall OK:", tostring(name))
	else
		print("[Pink's] safeCall ERROR:", tostring(name), "->", tostring(err))
	end
	return ok, err
end

print("[Pink's] Cargando servicios...")
local Players = game:GetService("Players")
local UserInputService = game:GetService("UserInputService")
local LocalPlayer = Players.LocalPlayer
local IsMobile = UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled
print("[Pink's] Servicios OK | Mobile =", tostring(IsMobile))

print("[Pink's] Cargando WindUI...")
local WindUI
local loadFails = {}
local loadFn = loadstring or load
if type(loadFn) ~= "function" then
	print("[Pink's] ERROR: loadstring/load no existe en este executor")
	return
end
if type(game.HttpGet) ~= "function" then
	print("[Pink's] ERROR: game.HttpGet no existe / HTTP deshabilitado")
	return
end

local function tryWind(url)
	print("[Pink's] WindUI try:", url)
	local ok, res = pcall(function()
		local src = game:HttpGet(url)
		assert(type(src) == "string" and #src > 200, "HttpGet vacio o corto (" .. tostring(type(src)) .. " len=" .. tostring(src and #src) .. ")")
		local fn, err = loadFn(src)
		assert(fn, "loadstring fail: " .. tostring(err))
		local lib = fn()
		assert(type(lib) == "table" and type(lib.CreateWindow) == "function", "lib invalida (no CreateWindow)")
		return lib
	end)
	if ok then
		print("[Pink's] WindUI OK desde:", url)
		return res
	end
	print("[Pink's] WindUI fail:", url, "->", tostring(res))
	table.insert(loadFails, tostring(res))
	return nil
end

WindUI = tryWind("https://github.com/Footagesus/WindUI/releases/latest/download/main.lua")
...
if not WindUI then
	print("[Pink's] WindUI FAILED total:\n" .. table.concat(loadFails, "\n"))
	return
end
print("[Pink's] WindUI cargado OK")
...
print("[Pink's] Notify inicial OK")
...
print("[Pink's] Player/Core OK | Me =", Me and Me.Name or "nil", "| OnPhone =", tostring(OnPhone))

print("[Pink's] Cargando managers/loops...")
local Loops = {}
local Conns = {}
local ConnGroups = {}
...
print("[Pink's] Cargando Farm state...")
local Farm = {
	on = false,
	state = "IDLE",
	doMach = false,
...
print("[Pink's] Cargando Bring state...")
local Bring = {
	on = false,
	tries = 10,
	coreyRoutine = true,
...
print("[Pink's] Cargando Util state...")
local Util = {
	fullbright = false,
	nofog = false,
	noclip = false,
...
print("[Pink's] Cargando Features state...")
local Features = {
...
print("[Pink's] Cargando ESP state...")
local ESP = {
...
print("[Pink's] Creando UI / Window...")
...
if not okWin or not Window then
	print("[Pink's] CreateWindow FAILED:", tostring(Window))
	toast("Window failed\n" .. tostring(Window), false)
	return
end
print("[Pink's] CreateWindow OK")

local Hub = Window

print("[Pink's] Creando Tag de version...")
pcall(function()
	Window:Tag({
		Title = VERSION,
		Icon = "github",
...
print("[Pink's] Creando Sections...")
local MainSection = Window:Section({ Title = "Main", Opened = true })
print("[Pink's] Section Main OK")
local ToolsSection = Window:Section({ Title = "Tools", Opened = true })
print("[Pink's] Section Tools OK")
local ExtraSection = Window:Section({ Title = "Extra", Opened = true })
print("[Pink's] Section Extra OK")

local SectionMap = {
	Home = MainSection,
	Visual = MainSection,
	Automatic = ToolsSection,
	Farms = ToolsSection,
	Config = ExtraSection,
}

local function Tab(title, desc, icon)
	print("[Pink's] Creando Tab:", tostring(title))
	local parent = SectionMap[title] or ToolsSection
	if not parent then
		print("[Pink's] Tab parent NIL para:", tostring(title))
		return nil
	end
	...
	if ok and r then
		print("[Pink's] Tab OK:", tostring(title))
		return r
	end
	print("[Pink's] Tab fallback:", tostring(title), "err=", tostring(r))
	local ok2, r2 = pcall(function()
		return parent:Tab({ Title = title, Icon = icon })
	end)
	if ok2 and r2 then
		print("[Pink's] Tab fallback OK:", tostring(title))
		return r2
	end
	print("[Pink's] Tab FAILED:", tostring(title), tostring(r2))
	return nil
end

local function Sec(tab, title, desc, icon)
	print("[Pink's] Creando Section:", tostring(title))
	if not tab then
		print("[Pink's] Section tab NIL:", tostring(title))
		return nil
	end
	local ok, r = pcall(function()
		return tab:Section({
	...
	if ok and r then
		print("[Pink's] Section OK:", tostring(title))
		return r
	end
	print("[Pink's] Section fallback:", tostring(title), "err=", tostring(r))
	local ok2, r2 = pcall(function()
		return tab:Section({ Title = title })
	end)
	if ok2 and r2 then
		print("[Pink's] Section fallback OK:", tostring(title))
		return r2
	end
	print("[Pink's] Section FAILED:", tostring(title), tostring(r2))
	return nil
end

local function Add(sec, method, data)
	if not sec then
		print("[Pink's] Add SKIP (sec nil):", tostring(method), data and data.Title)
		return nil
	end
	data = data or {}
	print("[Pink's] Add:", tostring(method), data.Title and ("Title=" .. tostring(data.Title)) or "")
	...
	if type(fn) ~= "function" then
		print("[Pink's] Add FAILED method no existe:", tostring(method))
		return nil
	end
	local ok, r = pcall(function() return fn(sec, data) end)
	if not ok then
		print("[Pink's] Add ERROR:", tostring(method), data.Title, "->", tostring(r))
	end
	return ok and r or nil
end

print("[Pink's] Helpers Tab/Sec/Add listos. Construyendo tabs...")
...
safeCall("UI:Automatic", function()
	local t = AutomaticTab or Tab("Automatic", "Instant systems", "solar:bolt-bold")
...
print("[Pink's] Script terminado OK | v" .. VERSION)
