...
function Utility:Diamond(parent, size, color, visible)
	local Diamond = Utility:Create("Frame", {
		BackgroundColor3 = color or Theme.Soul,
		BorderSizePixel = 0,
...
function DeltaUI:CreateWindow(config)
	...
	local SoulHolder = Utility:Create("Frame", {
		BackgroundTransparency = 1,
		Position = UDim2.new(0, 16, 0, 16),
		Size = UDim2.new(0, 18, 0, 18),
		Parent = Header,
	})
	local Soul = Utility:Diamond(SoulHolder, 14, Theme.Soul)
	task.spawn(function()
		while Soul.Parent do
			Utility:Tween(Soul, { Size = UDim2.new(0, 16, 0, 16) }, TweenInfo.new(0.5, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut))
			task.wait(0.5)
			Utility:Tween(Soul, { Size = UDim2.new(0, 12, 0, 12) }, TweenInfo.new(0.5, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut))
			task.wait(0.5)
		end
	end)
	...
	function Window:CreateTab(name)
		...
		local TabSoulHolder = Utility:Create("Frame", {
			BackgroundTransparency = 1,
			Position = UDim2.new(0, 8, 0.5, -5),
			Size = UDim2.new(0, 10, 0, 10),
			Parent = TabButton,
		})
		local TabSoul = Utility:Diamond(TabSoulHolder, 8, Theme.Soul, isFirstTab)
		...
		TabButton.MouseButton1Click:Connect(function()
			...
			if self.CurrentTab then
				...
				local prevSoulHolder = prev.Button:FindFirstChild("Frame")
				if prevSoulHolder then
					local prevSoul = prevSoulHolder:FindFirstChild("Frame")
					if prevSoul then prevSoul.Visible = false end
				end
				local prevLabel = prev.Button:FindFirstChildOfClass("TextLabel")
				if prevLabel then Utility:Tween(prevLabel, { TextColor3 = Theme.TextSecondary }, SnapTween) end
				prev.Page.Visible = false
			end

			Utility:Tween(TabButton, { BackgroundColor3 = Theme.ContainerLight }, SnapTween)
			TabButton.UIStroke.Thickness = 2
			TabSoul.Visible = true
		...
		function Tab:CreateButton(config)
			...
			local Soul = Utility:Diamond(SoulHolder, 8, Theme.Soul, false)
		...
		function Tab:CreateToggle(config)
			...
			local Fill = Utility:Diamond(Box, state and 14 or 0, Theme.Soul, state)
			...
			local function SetState(newState, fromInit)
				state = newState
				Fill.Visible = state
				Utility:Tween(Fill, { Size = state and UDim2.new(0, 14, 0, 14) or UDim2.new(0, 0, 0, 0) }, SnapTween)
		...
		function Tab:CreateDropdown(config)
			...
			local function BuildOptions()
				...
				for _, option in ipairs(options) do
					...
					local OptSoulHolder = Utility:Create("Frame", {
						BackgroundTransparency = 1,
						Position = UDim2.new(0, 8, 0.5, -4),
						Size = UDim2.new(0, 8, 0, 8),
						Parent = OptBtn,
					})
					Utility:Diamond(OptSoulHolder, 6, Theme.Soul, isSelected)
