...
do
    ...
    if type(f) == "function" then
        for k, v in pairs(S) do S[k] = f(v) end
    end
end

local Me = S.Players.LocalPlayer
if not Me then
	S.Players:GetPropertyChangedSignal("LocalPlayer"):Wait()
	Me = S.Players.LocalPlayer
end

local OnPhone = S.UIS.TouchEnabled and not S.UIS.KeyboardEnabled
local OnTV = S.GuiService:IsTenFootInterface()
...
local function Rem(name)
	...
	local r = S.RS:FindFirstChild(name)
	if okRemote(r) then
		remCache[name] = r
		return r
	end
	r = S.RS:FindFirstChild(name, true)
	if okRemote(r) then
		remCache[name] = r
		return r
	end
	for _, d in ipairs(S.RS:GetDescendants()) do
...
local function IsPlayerChar(m)
	...
	local pl = S.Players:GetPlayerFromCharacter(m)
	if pl then return true end
	if m:IsA("Model") and m:FindFirstChildOfClass("Humanoid") and S.Players:GetPlayerFromCharacter(m) then
		return true
	end
	return false
end
...
local function refreshCryptids(force)
	...
	pcall(function()
		for _, tag in ipairs({ "Cryptid", "Chaser", "Entity" }) do
			for _, o in ipairs(S.CS:GetTagged(tag)) do
				add(o)
			end
		end
	end)
	...
	for _, o in ipairs(S.Workspace:GetChildren()) do
		add(o)
		walk(o, 1)
	end
end
...
local function machineRoot(o)
	...
	while cur and cur ~= S.Workspace do
...
local function GetMachines()
	...
	pcall(function()
		for _, o in ipairs(S.CS:GetTagged("ArcadeMachine")) do push("ArcadeMachine", "Arcade", o) end
		for _, o in ipairs(S.CS:GetTagged("StorageMachine")) do push("StorageMachine", "Storage", o) end
		for _, o in ipairs(S.CS:GetTagged("CoreyMachine")) do push("CoreyMachine", "Corey", o) end
	end)
	...
	if not (hasA and hasS and hasC) then
		...
		pcall(function() walk(S.Workspace, 0) end)
	end
...
local function CompleteRemoteOnly(tag)
	...
	pcall(function()
		for _, m in ipairs(S.CS:GetTagged(tag)) do
			fire(m)
		end
	end)
...
pcall(function()
	local ev = S.RS:FindFirstChild("RoundEnderReady")
...
local function getSafePos()
	local mapY = S.Workspace:FindFirstChild("IntermissionMapYes")
...
local function isIntermission()
	local mapSv = S.RS:FindFirstChild("CurrentMap")
	...
	local area = S.RS:FindFirstChild("Area")
	if area then
...
local function isCoreyArea()
	...
	local gs = S.RS:FindFirstChild("Gamestate")
	if gs then
...
local function sessionNoteItems()
	pcall(function()
		local function scan(node, depth)
			...
			local tagged = false
			pcall(function()
				tagged = S.CS:HasTag(node, "Spawneditem") or S.CS:HasTag(node, "SpawnedItem")
			end)
...
local function readFloorNumber()
	...
	for _, name in ipairs({ "Floor", "CurrentFloor", "FloorNumber", "Floors" }) do
		local o = S.RS:FindFirstChild(name)
		if o then
...
local function isBringItem(o)
	...
	local tagged = false
	pcall(function() tagged = S.CS:HasTag(o, "Spawneditem") or S.CS:HasTag(o, "SpawnedItem") end)
	return tagged
end
...
local function bringItemOnce(safe)
	...
	pcall(function()
		for _, tag in ipairs({ "Spawneditem", "SpawnedItem" }) do
			for _, o in ipairs(S.CS:GetTagged(tag)) do consider(o) end
		end
	end)
	for _, o in ipairs(S.Workspace:GetChildren()) do consider(o) end
...
local function applyFullbright()
	pcall(function()
		S.Lighting.Brightness = 1.25
		S.Lighting.Ambient = Color3.fromRGB(95, 95, 100)
		S.Lighting.OutdoorAmbient = Color3.fromRGB(90, 90, 95)
		S.Lighting.GlobalShadows = true
		if S.Lighting.ClockTime < 5 or S.Lighting.ClockTime > 20 then
			S.Lighting.ClockTime = 12.5
		end
...
local function restoreLighting()
	...
	for k, v in pairs(lightSnap) do
		pcall(function() S.Lighting[k] = v end)
	end
	S.Lighting.GlobalShadows = true
end
...
local function applyNoFog()
	pcall(function()
		S.Lighting.FogEnd = math.max(S.Lighting.FogEnd, 2200)
		S.Lighting.FogStart = math.min(S.Lighting.FogStart, 80)
		for _, fx in ipairs(S.Lighting:GetChildren()) do
...
local function restoreFog()
	if fogSnap then
		local at = S.Lighting:FindFirstChildOfClass("Atmosphere")
		if at then
	...
	if lightSnap then
		S.Lighting.FogEnd = lightSnap.FogEnd
		S.Lighting.FogStart = lightSnap.FogStart
	end
end
...
local function startFly()
	...
	task.defer(function()
		...
		local bindChar = c
		Conn("fly", S.RunService.RenderStepped:Connect(function()
			if not flyWant or not Util.fly then return end
			local cur, hum, root, cam = Char(), Hum(), Root(), S.Workspace.CurrentCamera
			...
			local up = 0
			if S.UIS:IsKeyDown(Enum.KeyCode.Space) or S.UIS:IsKeyDown(Enum.KeyCode.ButtonR2) or S.UIS:IsKeyDown(Enum.KeyCode.ButtonA) then up += 1 end
			if S.UIS:IsKeyDown(Enum.KeyCode.LeftControl) or S.UIS:IsKeyDown(Enum.KeyCode.Q) or S.UIS:IsKeyDown(Enum.KeyCode.ButtonL2) then up -= 1 end
			if up ~= 0 then
...
local function processExistingDebuff(name)
	if not isDebuffEnabled(name) then return end
	local obj = S.Workspace:FindFirstChild(name, true)
	if obj then
...
local function startDebuffWatch()
	...
	debuffConns.child = S.Workspace.DescendantAdded:Connect(function(obj)
		...
		if obj.Name == "TouchInterest" or obj:IsA("TouchTransmitter") then
			...
			while parent and parent ~= S.Workspace do
...
local function startTPWalk()
	DisconnectKey("tw")
	if not TW.on then return end
	Conn("tw", S.RunService.Stepped:Connect(function(time, dt)
		twTick(time, dt)
	end))
end
...
local function hasLOS(fromPos, toPos)
	...
	local hit = S.Workspace:Raycast(fromPos, dir.Unit * math.min(dir.Magnitude, 400), params)
...
local function collectTeleportTargets(kind)
	...
	if kind == "Machines" then
		...
		pcall(function()
			for _, tag in ipairs({ "Spawneditem", "SpawnedItem" }) do
				for _, o in ipairs(S.CS:GetTagged(tag)) do add(o) end
			end
		end)
		for _, o in ipairs(S.Workspace:GetChildren()) do add(o) end
	end
	return out
end
			...
			Callback = function(v)
				...
				if not anyESP() then
					...
					if ESP.on.Items and not ESP.itemHooked then
						...
						Conn("espItems", S.Workspace.DescendantAdded:Connect(function(o)
						...
						Conn("espItemRemoved", S.Workspace.DescendantRemoving:Connect(function(o)
...
local x = Tools:Tab({
	...
	local function startImmunityObservers()
		if Immunity.StarGirl and not immStarConn then
			immStarConn = S.Workspace.DescendantAdded:Connect(function(obj)
		...
		if Immunity.Fan and not immFanConn then
			immFanConn = S.Workspace.DescendantAdded:Connect(function(obj)
	...
	local function stripImm()
		if Immunity.Triplets then pcall(function() local r = S.RS:FindFirstChild("TripletAlert"); if r then r:Destroy() end end) end
		if Immunity.Snuggles or Immunity.Dolly then pcall(function() local r = S.RS:FindFirstChild("SnugglesAlert"); if r then r:Destroy() end end) end
	...
	help:Button({
		...
		Callback = function()
			...
			local lp = game:GetService("Players").S.Players.LocalPlayer
			if not lp then
	...
	help:Toggle({
		...
		Callback = function(v)
			...
			setNoclip(true)
			Conn("noclip", S.RunService.Heartbeat:Connect(function()
...
pcall(function()
	if WindUI and WindUI.Notify then
		WindUI:Notify({ Title = "Pink's", Content = "v" .. C.Version .. " ready", Duration = 2, Icon = ASSETS.Notify })
	end
end)
